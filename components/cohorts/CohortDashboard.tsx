...
export default function CohortDashboard({ id }:{ id:string }){
  ...
  useEffect(()=>{ fetch(`/api/cohorts/${id}`).then(r=>r.json()).then(setData) },[id])
  ...
  return (
    <div className="space-y-4">
      ...
      <div><h3 className="font-semibold" style={{color:'var(--text-primary)'}}>Assignments ({data.assignments?.length||0})</h3>{data.assignments?.map((a:any)=><div key={a.id} className="card p-2 text-sm">{a.title} — {a.content_type}</div>) || <p style={{color:'var(--text-muted)'}}>No assignments</p>}</div>
    </div>
  )
}
