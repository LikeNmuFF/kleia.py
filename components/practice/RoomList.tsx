...
export default function RoomList({ rooms: initialRooms, isAdmin }: { rooms: PracticeRoom[]; isAdmin: boolean }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-6 p-6 sm:p-7 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em]" style={{ color: 'var(--accent)' }}>
              ...
              {isAdmin ? 'Practice workspace' : 'Invitation workspace'}
            </div>
            <h1 className="text-3xl font-bold tracking-tight sm:text-4xl" style={{ color: 'var(--text-primary)' }}>
              {isAdmin ? 'Build before you publish' : 'Your private practice rooms'}
            </h1>
            <p className="mt-3 max-w-xl leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              {isAdmin
                ? 'Create controlled testing rooms, invite selected testers, and promote challenges when they are ready.'
                : 'Work through challenges shared directly with you. Practice activity stays separate from global CTF scoring.'}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="rounded-xl border px-4 py-3 text-right" style={{ backgroundColor: 'var(--input-bg)', borderColor: 'var(--border-color)' }}>
              ...
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{rooms.length === 1 ? 'room' : 'rooms'}</p>
            </div>
            {isAdmin && (
      ...
      {message && <p role="status" className="rounded-xl border p-3 text-sm" style={{ borderColor: 'var(--border-color)', color: message === 'Room created.' ? '#22c55e' : '#ef4444' }}>{message}</p>}
      ...
      {open && (
        <form action={submit} className="rounded-2xl border p-5 sm:p-6" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
          <div className="mb-5">
            <h2 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>Create a testing room</h2>
            <p className="mt-1 text-sm" style={{ color: 'var(--text-muted)' }}>Only invited accounts will be able to find or enter this room.</p>
          </div>
          <div className="grid gap-4 md:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
            <label className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Room title<input required maxLength={120} name="title" className="input-field mt-1.5 w-full" placeholder="Web challenge review" /></label>
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            <button disabled={pending} className="btn-primary min-h-10 px-4 disabled:opacity-50">{pending ? 'Creating…' : 'Create private room'}</button>
          </div>
        </form>
      )}
      ...
      {rooms.length === 0 ? (
        <div className="rounded-2xl border px-6 py-16 text-center" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
          ...
          <h2 className="mt-4 font-semibold" style={{ color: 'var(--text-primary)' }}>No practice rooms available</h2>
          <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>{isAdmin ? 'Create the first invite-only room to begin testing.' : 'Private practice is invite-only. A room appears here after an admin invites you.'}</p>
        </div>
      ) : (
          ...
          {rooms.map((room) => (
            <article key={room.id} className="group flex min-h-64 flex-col rounded-2xl border p-5 transition duration-200 hover:-translate-y-0.5 sm:p-6" style={{ backgroundColor: 'var(--card-bg)', borderColor: 'var(--border-color)' }}>
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="rounded-full border px-2.5 py-1 text-[11px] font-medium" style={{ borderColor: 'var(--border-color)', color: 'var(--text-muted)' }}>Private</span>
              </div>
              ...
              <Link href={`/practice/${room.id}`} className="mt-5 inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border px-4 text-sm font-semibold transition group-hover:border-[var(--accent)]" style={{ borderColor: 'var(--border-color)', color: 'var(--text-primary)' }}>
                {isAdmin ? 'Manage room' : 'Enter room'}<ArrowRight className="h-4 w-4" aria-hidden />
              </Link>
            </article>
          ))}
        </div>
